...
public class HomeController {
    ...
    public String form1(@ModelAttribute FormModel fm, Model model) {
      ...
      model.addAttribute("member"," ご利用人数");
      model.addAttribute("name1"," ご依頼者名");
      model.addAttribute("name2"," ご依頼者名(フリガナ)");
      model.addAttribute("mail"," メールアドレス");
      model.addAttribute("address"," ご住所");
      model.addAttribute("error","の入力が不十分です");

      if(str1.matches("[0-9]")) model.addAttribute("member","");
      if(str2.matches("[一-龠々ぁ-んァ-ヶ 　]+")) model.addAttribute("name1","");
      if(str3.matches("[ァ-ヶ 　]+")) model.addAttribute("name2","");
      if(str4.matches("[a-zA-Z0-9@._-]+")) model.addAttribute("mail","");
      if(cnt==0){
        if(str5.equals("その他")) model.addAttribute("address","");
    ...
    public String confirmation(@ModelAttribute FormModel fm, Model model,HttpServletRequest request ) {
      String str1=fm.getInput1();
      String str2=fm.getInput2();
      String str3=fm.getInput3();
      String str4=fm.getInput4();
      ...
      str4=str4.replaceAll("(go{2,}gle)","google");
      str4=str4.replaceAll("(yahoo{2,})","yahoo");
